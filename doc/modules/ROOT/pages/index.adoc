...
* **Concurrent composition** via `when_all` for parallel task execution
...
* **Strand** serialization for concurrent executor access
* **Async mutex** for coroutine-friendly synchronization
...
* xref:coroutines/when-all.adoc[Concurrent Composition] — Run tasks in parallel with `when_all`
